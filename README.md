...
На текущей ветке исследование прошло Stage 1–4 и дошло до **Stage 5 — аудита proxy-сигнала `Q_B2` разрешёнными признаками**. Полный Stage 5 V1 выполнен и сохранён; его итоговый статус закрывается после review notebook и сверки summary.
...
Компактные проверяемые результаты завершённых этапов хранятся в `reports/summary/`.
...
После значимого Stage должны остаться notebook, machine-readable summary, ограничения, решение и следующий исследовательский вопрос. Это позволяет в конце собрать отчёт и презентацию из проверенных источников, а не восстанавливать историю по памяти или старым чатам.
