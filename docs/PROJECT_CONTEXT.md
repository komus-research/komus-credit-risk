...
Единственный current narrow controlled reopen — Stage 14 V1: `xRFM CPU_ONLY_RESEARCH_EXPERIMENT`, `XRFM_V1_LOCKED` / `READY_FOR_TECHNICAL_COORDINATOR`. Он проверяет iterative kernel / metric feature learning + supervised recursive localization на неизменном 47-feature contract. Frozen lock: `xrfm==0.4.5`, `device='cpu'`, 8 threads, `split_method='linear'`, один tree, `max_leaf_size=8192`, три leaf RFM iterations, no HPO, smoke → full Fold-1 feasibility → compute gate, manual review перед full OOF и ceiling 12 CPU wall-clock hours. GPU-only experiment не становится следующим основным Stage без отдельного нового решения пользователя; `STOPPED_BY_COMPUTE_COST` не является quality verdict. Data research, presentation и defence evidence остаются открытыми параллельными направлениями.
