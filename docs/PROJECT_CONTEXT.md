...
### Stage 4 — диагностика закрытых `Q_B1/Q_B2` — ЗАВЕРШЁН ДЛЯ ПЕРЕХОДА К STAGE 5
...
### Stage 5 — аудит proxy-сигнала `Q_B2` — ВЫПОЛНЕН, ИДЁТ REVIEW
...
Сохранённый статус summary:

`completed_pending_review`

Предварительные факты, которые должны быть подтверждены полным notebook review:
...
Предварительная интерпретация: текущие 47 признаков частично отражают общий сигнал `Q_B2`, но почти не воспроизводят ту часть сигнала, которая важна именно в общей blind spot. Это указывает на материальный недостающий информационный слой.

Окончательное решение по Stage 5 принимается только после review полного notebook и сверки notebook ↔ summary.
