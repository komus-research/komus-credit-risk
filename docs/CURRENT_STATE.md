...
Stage 14 V1 теперь: `xRFM CPU_ONLY_RESEARCH_EXPERIMENT`, Architect status `XRFM_V1_LOCKED` / `READY_FOR_TECHNICAL_COORDINATOR`. Проверяется одна новая hypothesis family: iterative kernel / metric feature learning + supervised recursive localization на тех же данных и сохранённом comparator. Lock: `xrfm==0.4.5`, `device='cpu'`, 8 threads, `split_method='linear'`, `n_trees=1`, `max_leaf_size=8192`, leaf RFM iterations=3, no HPO; smoke → full Fold-1 feasibility → compute gate; full OOF только после отдельного manual review; ceiling 12 CPU wall-clock hours. `STOPPED_BY_COMPUTE_COST` не является quality verdict.

`CORE_MODEL_RESEARCH_STOPPED_CURRENT_47_FEATURES` остаётся базовым решением для обычного architecture search. Разрешён только этот narrow controlled reopen. Сохраняются `Data_final.xlsb`, его SHA, 289614 working rows, `DefMark`, исключение `INN`, exact accepted 47 features, запрет `Q_B1_norm`/`Q_B2_norm` как predictors, те же 3 folds seed 42, saved `GBDT_mean` Gini 0.8063993952 и закрытый final test. При xRFM `no_material_benefit`/`inferior` default direction — data / feature / blind-spot research, не новая модель.
